...
EVIDENCE_URL = "https://evidence.example/unavailable.json"
TOTAL = 1_000
BOND = 25


def deploy_deal(direct_vm, direct_deploy, buyer, seller, platform):
    ...
    deal_id = contract.open_deal(Address(seller), MILESTONES_URL, TOTAL, False)
    direct_vm.value = 0
    direct_vm.sender = seller
    contract.accept_deal(deal_id)
    contract.declare_milestone(deal_id, 0, "Deliver the audited package", 10_000)
    return contract, deal_id


def open_dispute(direct_vm, contract, deal_id, buyer):
    direct_vm.mock_llm(
        r".*Normalize a free-text dispute statement.*",
        json.dumps({"summary": "The submitted package does not satisfy the milestone."}),
    )
    direct_vm.sender = buyer
...
def test_open_deal_requires_exact_funding(
    ...
    with direct_vm.expect_revert("must equal total"):
        ...
        contract.open_deal(Address(direct_bob), MILESTONES_URL, TOTAL, False)


def test_only_seller_can_claim_released_tranche_and_accounting_updates(
    direct_vm, direct_deploy, direct_alice, direct_bob, direct_owner
):
    ...
    direct_vm.sender = direct_alice
    assert contract.release_milestone(deal_id, 0) == TOTAL

    before = contract.deal(deal_id)
    assert before["allocated"] == TOTAL
    assert before["released"] == 0
    assert before["seller_claimable"] == TOTAL
    ...
    after = contract.deal(deal_id)
    assert after["seller_claimable"] == 0
    assert after["released"] == TOTAL
    assert after["refunded"] == 0
    assert after["escrow_balance"] == 0
...
def test_timeout_recovery_refunds_buyer_and_returns_bond(
    direct_vm, direct_deploy, direct_alice, direct_bob, direct_owner
):
    direct_vm.warp("2026-07-01T00:00:00Z")
    contract, deal_id = deploy_deal(
    ...
    final_state = contract.deal(deal_id)
    assert final_state["refunded"] == TOTAL
    assert final_state["escrow_balance"] == 0


def test_unavailable_evidence_does_not_block_buyer_wins_settlement(
    direct_vm,
    direct_deploy,
    direct_alice,
    direct_bob,
    ...
    contract.assign_arbiter(deal_id, Address(direct_charlie))
    direct_vm.sender = direct_charlie
    contract.file_arbiter_finding(
        deal_id,
        dispute_id,
        "BUYER_WINS",
        "The package is incomplete and should be refunded.",
    )

    direct_vm.mock_web(
        r".*evidence\.example/unavailable\.json",
    ...
    direct_vm.mock_llm(
        ...
        json.dumps(
            {
                "verdict": "BUYER_WINS",
                # A contradictory model field must not override the categorical
                # winner; the contract canonicalizes BUYER_WINS to zero.
                "split_bps": 5_000,
                "reasoning": "The arbiter finding supports a full refund.",
            }
        ),
    )
    assert contract.finalize_dispute(deal_id, dispute_id) == "BUYER_WINS"
