...
export const recoverTimedOutDispute = (
  account: Hex,
  dealId: string,
  disputeId: string
) => send(account, "recover_timed_out_dispute", [dealId, disputeId]);

export const claimSellerPayout = (account: Hex, dealId: string) =>
  send(account, "claim_seller_payout", [dealId]);

export const claimBuyerRefund = (account: Hex, dealId: string) =>
  send(account, "claim_buyer_refund", [dealId]);

export const claimDisputeBond = (account: Hex, disputeId: string) =>
  send(account, "claim_dispute_bond", [disputeId]);
