...
export function Actions({ account, deal, milestones, busy, run, onDisputeOpened }: Props) {
  ...
  const [recoveryDispute, setRecoveryDispute] = useState("");
  const [bondDispute, setBondDispute] = useState("");
  ...
  return (
    <div className={s.grid}>
      ...
      <Group role="ANY" title="Finalize the verdict">
        ...
        <input
          className={`${s.in} mono`}
          value={recoveryDispute}
          onChange={(e) => setRecoveryDispute(e.target.value)}
          placeholder="timed-out dispute_id"
        />
        <button
          className="btn red"
          disabled={!can || !recoveryDispute}
          onClick={() =>
            run("Recovering timed-out dispute", () =>
              svc.recoverTimedOutDispute(account, id, recoveryDispute)
            )
          }
        >
        ...
        <input
          className={`${s.in} mono`}
          value={bondDispute}
          onChange={(e) => setBondDispute(e.target.value)}
          placeholder="resolved dispute_id"
        />
        <button
          className="btn"
          disabled={!can || !bondDispute}
          onClick={() =>
            run("Claiming dispute bond", () =>
              svc.claimDisputeBond(account, bondDispute)
            )
          }
        >
          Claim awarded bond
