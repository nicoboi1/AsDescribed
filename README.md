...
6. Settle: the verdict prepares exact seller and buyer claimable balances. The authorized winner then pulls the GEN with `claim_seller_payout` or `claim_buyer_refund`.
7. Recover: after seven days, either party can close a stalled dispute deterministically. The disputed tranche becomes refundable to the buyer and the dispute bond becomes reclaimable by its depositor, so a dead evidence URL cannot freeze funds forever.
...
A single basis-point split drives both the jury verdict and the on-chain allocation. Settlement uses checks-effects-interactions: claimable state is zeroed and accounting is updated before `emit_transfer` queues the GEN transfer.

`released` now means GEN actually claimed by the seller. `refunded` means GEN actually reclaimed by the buyer. `allocated` tracks resolved escrow that has been committed to either side, while `escrow_balance` reports funds still held by the contract.
...
| `release_milestone` | write | Buyer approves a requested tranche and makes it claimable by the seller. |
...
| `recover_timed_out_dispute` | write | After seven days, resolves a stalled dispute to a buyer refund without fetching its evidence URL. |
| `claim_seller_payout` | write | Seller-only pull payment that transfers awarded GEN and updates released accounting. |
| `claim_buyer_refund` | write | Buyer-only pull refund that transfers awarded GEN and updates refund accounting. |
| `claim_dispute_bond` | write | Transfers a resolved dispute bond to the verdict-authorized recipient. |
| `void_deal` | write | Platform voids a stalled deal and makes unallocated escrow claimable by the buyer. |
...
The direct suite covers exact funding, seller-only payout, buyer-only refund, early-timeout rejection, seven-day recovery, bond recovery, double-claim protection, accounting, and unavailable evidence. The GLSim integration smoke test verifies the emitted seller transfer and post-transfer accounting; run it on Linux/WSL because GLSim 0.29.2 has an fd0 temporary-file locking limitation on Windows.
